...
export default function Footer() {
  return (
    <footer className="bg-black w-full border-t border-[#262626]">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-12 px-6 md:px-12 py-16 w-full">
        {/* Brand */}
        <div className="md:col-span-1">
          <Link
            href="/"
            className="font-headline text-xl font-black text-white mb-8 block"
          >
            BLACK BULLET
          </Link>
            ...
            {["ECU Remapping", "Carbon Fiber Kits", "Exhaust Systems"].map(
              (item) => (
                <li key={item}>
                  <Link
                    href="/services"
                    className="font-body text-sm text-outline-variant hover:text-primary transition-all underline-offset-4 hover:underline"
                  >
                    {item}
                  </Link>
                </li>
      ...
      {/* Bottom Bar */}
      <div className="px-6 md:px-12 py-6 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="font-body text-xs text-outline-variant">
          © 2024 BLACK BULLET GARAGE PERFORMANCE DUBAI. ALL RIGHTS RESERVED.
        </div>
        <div className="flex gap-6">
          <span className="material-symbols-outlined text-outline-variant text-lg hover:text-primary transition-all cursor-pointer">
            share
          </span>
          <span className="material-symbols-outlined text-outline-variant text-lg hover:text-primary transition-all cursor-pointer">
            location_on
